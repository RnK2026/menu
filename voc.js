...
export function validateReplyDraft(input) {
  ...
  return { author, content, asAdmin, password: asAdmin ? password : '' };
}
...
function validateReplies(post) {
  ...
  return source.map(reply => {
    ...
    return { id:reply.id, author, content, createdAt:reply.createdAt, isAdmin };
  });
}
...
export function validateVocList(value) {
  ...
  return value.map(post => {
    ...
    const content = validateVocDraft(post);
    const replies = validateReplies(post);
    return {...content,id:post.id,createdAt:post.createdAt,replies,status:replies.length?`답글 ${replies.length}개`:'접수',hasPhoto:Boolean(post.hasPhoto),photo:validatePhotoPayload(post.photo)};
  });
}
